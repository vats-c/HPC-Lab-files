/* Experiment 2 – Approach2 (Immediate) Replacement: Parallel Scalability
...
int main() {
  ...
  printf(
      "=== Experiment 2: Approach2 (Immediate Replacement) – Parallel Scalability ===\n");
  printf("NX,NY,Threads,Serial_Mover,Parallel_Mover,Speedup,Serial_Total,"
         "Parallel_Total\n");
  ...
  for (int g = 0; g < n_grids; g++) {
    ...
    for (int t = 0; t < n_threads; t++) {
      ...
      for (int it = 0; it < Maxiter; it++) {
        ...
        interpolation(mesh, pts);
        double t1 = omp_get_wtime();
        mover_parallel_Approach2(pts, dx, dy);
        double t2 = omp_get_wtime();
        par_interp += (t1 - t0);
        par_mover += (t2 - t1);
      }
      ...
      printf("%d,%d,%d,%lf,%lf,%lf,%lf,%lf\n", NX, NY, thread_counts[t],
             serial_mover, par_mover, speedup, serial_total, par_total);
      free(mesh);
      free(pts);
    }
  }
