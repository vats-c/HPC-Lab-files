/* Experiment 1 – Approach1 (Deferred) Insertion Approach
 * Serial mover scaling with particle count across 3 grid configs.
 * Output: CSV with total execution times (interp + mover summed over Maxiter).
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main() {
    ...
    /* Add 1000000000 (1e9) if enough RAM (requires ~16 GB) */
    int n_pc = 4;  /* change to 5 if including 1e9 */
    int n_grids = 3;
